-- Migration: Make upsert_repos idempotent against duplicate github_id values
-- in the same payload.
--
-- Why: GitHub's paginated /user/starred can transiently return the same repo
-- on two consecutive pages (e.g. if you star/unstar mid-walk, or if the
-- underlying stargazers view shifts). The input then has two rows with the
-- same github_id, and Postgres' ON CONFLICT raises
--   21000: ON CONFLICT DO UPDATE command cannot affect row a second time.
--
-- Fix: DISTINCT ON deduplicates in SQL before the INSERT sees the rows. The
-- first occurrence wins — stable given GitHub's page ordering (most recently
-- starred first).
set
  search_path = public,
  extensions;
...
begin
  ...
  input as (
    -- Dedupe on github_id. Keep the first occurrence (lowest ord) —
    -- GitHub returns most-recently-starred first, so earlier == fresher.
...
comment on function star_vault.upsert_repos (jsonb, bigint) is 'Bulk upsert starred repos; deduplicates on github_id in-place. Returns (added, updated) via xmax=0. If run_id is provided, tags each row with seen_at = run_id for reconciliation.';
